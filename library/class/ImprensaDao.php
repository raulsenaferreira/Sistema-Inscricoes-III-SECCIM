...
class ImprensaDao extends Dao{
 ...
 {
	$sql = "INSERT INTO {$this->table} (nome,tipo,link,target,extensao,datapub) 
	 	 values ('".$dados['nome']."','".$dados['tipo']."','".$dados['link']."','".$dados['target']."','".$dados['extensao']."','".$dados['datapub']."')";
	 $conn = new Database();
 ...
 {
	 $sql = "UPDATE {$this->table} set nome = '".$dados['nome']."', tipo = '".$dados['tipo']."', link = '".$dados['link']."', target = '".$dados['target']."', 
	 	 datapub = '".$dados['datapub']."', datapub = '".$dados['datapub']."' WHERE id = {$id}";
	 $conn = new Database();
	 return $conn->execute($sql);
 }
